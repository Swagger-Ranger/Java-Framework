...
<body class="childrenBody">
		...
		<blockquote class="layui-elem-quote news_search">
			<form id="form1" class="layui-form" method="post" action="AdminRechargeLogServlet?op=adminRechargeLogList&comp="${comp }">
					...
					<input style="width: 10%;display:inline;" type="text" id="money" name="money" value="${money }" lay-verify="required" placeholder="根据充值金额" autocomplete="off" class="layui-input">
					<input style="width: 13%;display:inline;" type="text" id="aid" name="aid" value="${aid }" lay-verify="required" placeholder="根据管理员ID查询" autocomplete="off" class="layui-input">
					...
					<div style="width: 15%;display:inline;" class="layui-form-item">
						<div class="layui-input-inline">
							<select name="rtype">
								<option ${rtype != '1' and rtype!= '2' and rtype!= '3' ? 'selected':' '} value="">根据充值类型查询</option>
								<option ${rtype == '1' ?'selected':' ' } value="1">金币充值</option>
								<option ${rtype == '2' ?'selected':' ' } value="2">钻石充值</option>
								<option ${rtype == '3' ?'selected':' ' } value="3">积分充值</option>
							</select>
						</div>
					</div>	
			</form>
				...
				<c:forEach var="pml" items="${pm.list}" varStatus="status">
								<tr>
									...
									<c:forEach var="u" items="${ulist}" varStatus="status">
										<c:if test="${u.id==pml.uid }">
											<td>ID[${u.id }]&nbsp;${u.nickname }</td>
										</c:if>
									</c:forEach>
									<td>${pml.money}</td>
									<td>${pml.rtype==1?'金币充值':pml.rtype==2?'钻石充值':'积分充值'}</td>
									<c:forEach var="a" items="${alist}" varStatus="status">
										<c:if test="${a.id==pml.aid }">
											<td>ID[${a.id }]&nbsp;${a.adminname }</td>
										</c:if>
									</c:forEach>
	...
	layui.use(['form','layer','laydate'],function(){
		...
		$("#reset").click(function(){
			...
			window.location.href="AdminRechargeLogServlet?op=adminRechargeLogList&currentPage=1&pageSize="+pageSize+"&comp="+comp;
		});
	</script>
</body>
</html>