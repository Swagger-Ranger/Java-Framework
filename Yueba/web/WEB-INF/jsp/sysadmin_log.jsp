...
<body class="childrenBody">
		...
		<blockquote class="layui-elem-quote news_search">
			<form id="form1" class="layui-form" method="post" action="SystemLogServlet?op=loglist">
					...
					<input style="width: 15%;display:inline;"  type="text" id="aid" name="aid" value="${aid }"  placeholder="根据管理员ID查询" autocomplete="off" class="layui-input">
					...
					<div style="width: 20%;display:inline;" class="layui-form-item">
						<div class="layui-input-inline">
							<select name="operating">
								<option ${operating != '发送及时公告' and operating!= '发送邮件-普通玩家(群体)' and operating!='发送邮件-推广员玩家(群体)' ? 'selected':' '} value="">根据审核状态查询</option>
								<option ${operating == '发送及时公告' ?'selected':' ' } value="发送及时公告">发送及时公告</option>
								<option ${operating == '发送邮件-普通玩家(群体)' ?'selected':' ' } value="发送邮件-普通玩家(群体)">发送邮件-普通玩家(群体)</option>
								<option ${operating == '发送邮件-推广员玩家(群体)' ?'selected':' ' } value="发送邮件-推广员玩家(群体)">发送邮件-推广员玩家(群体)</option>
							</select>
						</div>
					</div>
			</form>
	...
	layui.use(['form','layer','laydate'],function(){
		...
		$("#reset").click(function(){
			...
			window.location.href="SystemLogServlet?op=loglist&pageSize="+pageSize+"&currentPage=1";
		});
	</script>
</body>
</html>