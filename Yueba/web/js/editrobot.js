...
layui.config({
	...
	var layer = parent.layer === undefined ? layui.layer : parent.layer;
        ...
        loadProvince(); //加载省信息
        var parameter = window.parent.obj;
		$(".account").val(parameter.account);
		$(".robotId").val(parameter.id);
		$(".realName").val(parameter.nickname);
		$(".myself").text(parameter.autograph);
		$("#userFace").attr("src",parameter.head);
   layui.upload({
   ...
   /* if(window.sessionStorage.getItem('userFace')){
    ...
    form.on("submit(changeUser)",function(data){
        ...
        userInfoHtml = {
            ...
            'myself' : $(".myself").val(),
            'userFace' : $("#userFace")[0].src
        };
        $.post("RobotServlet",userInfoHtml,function(e){
        	setTimeout(function(){
