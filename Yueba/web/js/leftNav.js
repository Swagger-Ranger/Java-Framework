		...
		if(data[i].children != undefined && data[i].children.length > 0){
			...
			for(var j=0;j<data[i].children.length;j++){
				if(data[i].children[j].spread){
					ulHtml += '<li class="layui-nav-item2 layui-nav-itemed2">';
				}else{
					ulHtml += '<li class="layui-nav-item2">';
				}
				if(data[i].children[j].children != undefined && data[i].children[j].children.length > 0){
					...
					ulHtml += '<span class="layui-nav-more2"></span>';
					ulHtml += '</a>';
					ulHtml += '<dl class="layui-nav-child2">';
					for(var k=0;k<data[i].children[j].children.length;k++){
						...
						if(data[i].children[j].target == "_blank"){
							ulHtml += '<a href="javascript:;" data-url="'+data[i].children[j].href+'" target="'+data[i].children[j].target+'">';
						}else{
							ulHtml += '<a href="javascript:;" data-url="'+data[i].children[j].href+'">';
						}
						if(data[i].children[j].icon != undefined && data[i].children[j].icon != ''){
							if(data[i].children[j].icon.indexOf("icon-") != -1){
								ulHtml += '<i class="iconfont '+data[i].icon+'" data-icon="'+data[i].children[j].icon+'"></i>';
							}else{
								ulHtml += '<i class="layui-icon" data-icon="'+data[i].children[j].icon+'">'+data[i].children[j].icon+'</i>';
							}
						}
